...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
	...
	let mut grid = Grid::new(len_seq1 + 1, len_seq2 + 1);

	for i in 0..len_seq1+1 {
		let _ = grid.set(i, 0, 0);
	}
	for j in 0..len_seq2+1 {
	...
	for i in 0..len_seq1 {
			for j in 0..len_seq2 {
					if seq1[i] == seq2[j] {
							...
							let _ = grid.set(i + 1, j + 1, cmp::max(grid.get(i + 1, j).unwrap(), grid.get(i, j + 1).unwrap()));
					}
			}
	}
	return grid;
...
fn main() {
	...
	let vec1 = read_file_lines(filename1).unwrap();
	let vec2 = read_file_lines(filename2).unwrap();
