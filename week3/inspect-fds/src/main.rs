...
fn main() {
	...
	let target_get = ps_utils::get_target(&target).expect("Error when calling ps or pgrep");
	match target_get {
			Some(process) => {
			//	println!("Found pid {}", process.pid);
				process.print();
				for child in ps_utils::get_child_processes(process.pid).expect("Faild to get child processes") {
					// 	println!("Found pid {}", child.pid)
						child.print();
				}
			},
			None => {
